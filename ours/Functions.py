...
class Mlp(nn.Module):
    ...
    def _init_weights(self, m): #权重初始化
        if isinstance(m, nn.Conv2d):
            trunc_normal_(m.weight, std=.02)   #使用标准差为 0.02 的截断正态分布初始化卷积层的权重。
            if m.bias is not None:
                nn.init.constant_(m.bias, 0)  #将卷积层的偏差初始化为零。
...
if __name__ == '__main__':
    model = Mlp(32)
    input_tensor = torch.randn(6, 32, 8, 8)
